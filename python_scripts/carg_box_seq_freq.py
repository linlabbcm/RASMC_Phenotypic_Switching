...
freq_table=[]
header=['TF_NAME','FREQ_SUM','NUM_REGIONS','MOTIF/KB','STD_DEV']
...
#               print(positions)
            ...
            seq_density = float((len(positions)/len(line))*10)
#            print(seq_density)
...
#            print(seq_density)
        ...
        for line in table[1:]:
            ...
            freq_list.append((float(line[2])*10/float(line[3])))
#            print(freq_sum)
        freq_std=numpy.std(freq_list)
        freq_tab_line=[tf_name,freq_sum,tab_len,(freq_sum/tab_len)*1000,freq_std]
        freq_table.append(freq_tab_line)
        fasta_path='%s%s.fasta' % (temp_dir,tf_name)
        utils.unParseTable(fasta,fasta_path,'')
        
        print(tf_name,temp_dir,fasta_path)
        bg_path = makeMotifBackground(fasta_path,temp_dir,tf_name)
